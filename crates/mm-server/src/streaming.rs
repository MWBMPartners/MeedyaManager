...
impl StreamRequest {
    ...
    pub fn is_range_request(&self) -> bool {
        !matches!(self, StreamRequest::Full)
    }
    ...
    pub fn resolve(&self, size: u64) -> Option<(u64, u64)> {
        match self {
            StreamRequest::Full => {
                if size == 0 { None } else { Some((0, size - 1)) }
            }
            StreamRequest::Range { start, end } => {
...
impl RangeParser {
    ...
    pub fn parse(header: &str) -> Result<StreamRequest, StreamError> {
        ...
        let spec = header
            .strip_prefix("bytes=")
            .ok_or_else(|| StreamError::MalformedRange { header: header.to_string() })?;

        if let Some(suffix_str) = spec.strip_prefix('-') {
            // Suffix range: bytes=-N
            let n: u64 = suffix_str
                .parse()
                .map_err(|_| StreamError::MalformedRange { header: header.to_string() })?;
            return Ok(StreamRequest::Suffix { suffix: n });
        }

        let parts: Vec<&str> = spec.splitn(2, '-').collect();
        if parts.len() != 2 {
            return Err(StreamError::MalformedRange { header: header.to_string() });
        }
...
impl MediaStreamer {
    ...
    pub fn content_type(path: &str) -> &'static str {
        ...
        if lower.ends_with(".opus")  { return "audio/opus"; }
        if lower.ends_with(".wav")   { return "audio/wav"; }
        if lower.ends_with(".aiff")  { return "audio/aiff"; }
    ...
    pub fn prepare_response(
        ...
        let resolved = range.resolve(file_size).ok_or_else(|| {
        ...
        if range.is_range_request() {
            Ok(StreamResponse::partial(resolved.0, resolved.1, file_size, content_type))
        } else {
...
mod tests {
    ...
    fn stream_config_valid_when_root_set() {
        let cfg = StreamConfig { media_root: "/media".into(), ..StreamConfig::default() };
        assert!(cfg.is_valid());
    }
    ...
    fn parse_full_range() {
        ...
        assert_eq!(r, StreamRequest::Range { start: 0, end: 1023 });
    }
    ...
    fn resolve_range_request() {
        let (s, e) = StreamRequest::Range { start: 100, end: 199 }.resolve(1000).unwrap();
        assert_eq!((s, e), (100, 199));
    }

    #[test]
    fn resolve_out_of_range_returns_none() {
        let r = StreamRequest::Range { start: 900, end: 1100 }.resolve(1000);
    ...
    fn byte_count_for_range() {
        let count = StreamRequest::Range { start: 0, end: 99 }.byte_count(1000).unwrap();
        assert_eq!(count, 100);
    }
    ...
    fn content_type_mapping() {
        assert_eq!(MediaStreamer::content_type("/music/track.flac"), "audio/flac");
        assert_eq!(MediaStreamer::content_type("/video/film.mp4"),   "video/mp4");
        assert_eq!(MediaStreamer::content_type("/misc/data.bin"),    "application/octet-stream");
        assert_eq!(MediaStreamer::content_type("/music/album.mp3"),  "audio/mpeg");
        assert_eq!(MediaStreamer::content_type("/stream/list.m3u8"), "application/vnd.apple.mpegurl");
    }
    ...
    fn prepare_response_range_not_satisfiable() {
        let range = StreamRequest::Range { start: 5000, end: 6000 };
        let err = streamer()
    ...
    fn prepare_response_file_too_large() {
        ...
        let err = s.prepare_response("big.mkv", &StreamRequest::Full, 200).unwrap_err();
    ...
    fn stream_error_display_messages() {
        let e = StreamError::FileNotFound { path: "/a.mp3".into() };
        assert!(e.to_string().contains("/a.mp3"));
        let e = StreamError::PathTraversalDenied { path: "../etc".into() };
